...
日常照片纠错和允许推测的网页演示，先按[室内建模准确性与快速交付 SOP](室内建模准确性与快速交付SOP.zh-CN.md)选择展示分支。涉及建筑外观、驾驶、宣传物、多语言或服务器同步时，按需使用[展示精修与增量交付流程](展示精修与增量交付流程.zh-CN.md)。展示验证不替代下方权威测量流程及其门禁。
